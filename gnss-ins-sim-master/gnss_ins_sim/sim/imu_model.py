...
odo_low_accuracy = {'scale': 0.99,
                    'stdv': 0.1}
# CORRECTED JZ105 RLG MODEL - Following the exact pattern
#gyro_jz105_rlg = {
    # Bias (use constant bias)
 #   'b': np.array([0.0, 0.0, 0.0]) * D2R / 3600.0,

    # Bias instability → model as drift
  #  'b_drift': np.array([0.1, 0.1, 0.1]) * D2R / 3600.0,

    # Correlation time 
   # 'b_corr': np.array([100.0, 100.0, 100.0]),

    # ARW: already in deg/√s → just convert to rad
    #'arw': np.array([0.06, 0.06, 0.06]) * D2R / 60.0,
    
#}


# JZ105 RLG Model (Ring Laser Gyro)
# Typical specifications for navigation-grade RLG:
# - Gyro bias: 0.0 deg/hr (Actual bias)
# - ARW: 0.06 deg/√hr  (ok)
# - Gyro Bias stability: 0.1 deg/hr (ok)
# - Accel bias: 0 g (Actual bias)
# - Accel VRW: 5 μg/√s (ok)
# - Accel Bias stability: 20ug (ok)
gyro_jz105_rlg = {
    'b': np.array([0.0, 0.0, 0.0]) * D2R / 3600.0,      # deg/hr -> rad/s 
    'b_drift': np.array([0.1, 0.1, 0.1]) * D2R / 3600.0,   # deg/hr -> rad/s	(ok)									
    'b_corr': np.array([100.0, 100.0, 100.0]),          # seconds (RLG typical) 
    'arw': np.array([0.06, 0.06, 0.06]) * D2R / 60.0,      # deg/√hr -> rad/√s (ok)
}
accel_jz105_rlg = {
    'b': np.array([0.0e-6 * 9.80665]*3),
    'b_drift': np.array([20e-6 * 9.80665]*3),
    'b_corr': np.array([100.0, 100.0, 100.0]),
    'vrw': np.array([5e-6 * 9.80665]*3),
...
class IMU(object):
    ...
    def __init__(self, accuracy='low-accuracy', axis=6,
        ...
        Args:
            accuracy: IMU grade.
                This can be a string to use the built-in IMU models:
                    ...
                    'mid-accuracy':
                    'high-accuracy':
                    'jz105-rlg':    # ADDED: JZ105 RLG model
        ...
        if isinstance(accuracy, str):
            ...
            # ADDED: JZ105 RLG model case
            elif accuracy == 'jz105-rlg':
                self.gyro_err = gyro_jz105_rlg
                self.accel_err = accel_jz105_rlg
                # Use default mag error if magnetometer is enabled
