...
#
import sys, os, platform, time, re, shutil, csv
...
# Optional serial interface for sending TCA-ready GNSS measurements to Teensy.
...
opts_file  = OPTS_FILE                 # active options file
...
# Optional GNSS/IMU launch synchronization -------------------------------------
# Normally set by run_pocket_imu_synced.py. Without these variables, PocketSDR
# retains its original manual-start behavior.
POCKETSDR_AUTOSTART = os.environ.get(
    'POCKETSDR_AUTOSTART', '0'
).strip().lower() not in ('0', 'false', 'off', 'no', '')

POCKETSDR_SIM_READY_FILE = os.environ.get(
    'POCKETSDR_SIM_READY_FILE', ''
).strip()

# First-sample synchronization files. PocketSDR publishes the first valid
# TCA observation epoch, waits for the IMU simulator to align its precomputed
# arrays to that epoch, then creates GO and transmits the held first TCA packet.
POCKETSDR_SIM_EPOCH_FILE = os.environ.get(
    'POCKETSDR_SIM_EPOCH_FILE', ''
).strip()

POCKETSDR_SIM_ALIGNED_FILE = os.environ.get(
    'POCKETSDR_SIM_ALIGNED_FILE', ''
).strip()

POCKETSDR_SIM_GO_FILE = os.environ.get(
    'POCKETSDR_SIM_GO_FILE', ''
).strip()

try:
    POCKETSDR_SIM_SYNC_POLL_MS = max(
        1, int(os.environ.get('POCKETSDR_SIM_SYNC_POLL_MS', '10'))
    )
except ValueError:
    POCKETSDR_SIM_SYNC_POLL_MS = 10

# TCA serial output options -----------------------------------------------------
# Set POCKETSDR_TCA_PORT to enable transmission, for example:
#   Windows : set POCKETSDR_TCA_PORT=COM5
#   Linux   : export POCKETSDR_TCA_PORT=/dev/ttyACM0
TCA_SERIAL_PORT = os.environ.get('POCKETSDR_TCA_PORT', '').strip()
TCA_SERIAL_ENABLE = os.environ.get(
    'POCKETSDR_TCA_ENABLE', '1' if TCA_SERIAL_PORT else '0'
).strip().lower() not in ('0', 'false', 'off', 'no', '')
try:
    TCA_SERIAL_BAUDRATE = int(os.environ.get('POCKETSDR_TCA_BAUDRATE', '500000'))
except ValueError:
    TCA_SERIAL_BAUDRATE = 500000
try:
    TCA_TX_MIN_INTERVAL = float(os.environ.get('POCKETSDR_TCA_TX_INTERVAL', '0.0'))
except ValueError:
    TCA_TX_MIN_INTERVAL = 0.0
try:
    TCA_SERIAL_CYCLE = int(os.environ.get('POCKETSDR_TCA_SERIAL_CYCLE_MS', '5'))
except ValueError:
    TCA_SERIAL_CYCLE = 5

# Teensy returns processed TCA/INS solution at high rate. Read frequently and
# keep only the latest complete solution for GUI display/outage-error use.
try:
    TCA_SERIAL_RX_CYCLE = int(os.environ.get('POCKETSDR_TCA_SERIAL_RX_CYCLE_MS', '5'))
except ValueError:
    TCA_SERIAL_RX_CYCLE = 5
try:
    TCA_SERIAL_RX_MAX_BYTES = int(os.environ.get('POCKETSDR_TCA_SERIAL_RX_MAX_BYTES', '32768'))
except ValueError:
    TCA_SERIAL_RX_MAX_BYTES = 32768
TCA_SERIAL_RETURN_DEBUG = os.environ.get('POCKETSDR_TCA_RETURN_DEBUG', '0').strip().lower() \
    not in ('0', 'false', 'off', 'no', '')
try:
    _tca_write_timeout = os.environ.get('POCKETSDR_TCA_WRITE_TIMEOUT', '').strip()
    TCA_SERIAL_WRITE_TIMEOUT = None if _tca_write_timeout == '' else float(_tca_write_timeout)
except ValueError:
    TCA_SERIAL_WRITE_TIMEOUT = None

# TCA manual outage test options ----------------------------------------------
TCA_OUTAGE_DURATION = 20.0      # seconds; each manual outage auto-ends
TCA_OUTAGE_SAT_OPTIONS = ('0', '1', '2', '3')
TCA_NORMAL_SAT_OPTIONS = tuple(str(i) for i in range(21))  # normal TCA TX count, 0..20


# TCA/GNSS PVT CSV output options ---------------------------------------------
# These can be changed from the Output... button while the receiver is stopped.
TCA_PVT_CSV_HEADER = (
    'wall_time_unix', 'wall_time_local', 'gnss_time_gpst',
    'gnss_lat_deg', 'gnss_lon_deg', 'gnss_h_m',
    'tca_time', 'tca_lat_deg', 'tca_lon_deg', 'tca_h_m'
)
TCA_PVT_CSV_DEFAULT_PATH = os.environ.get(
    'POCKETSDR_TCA_PVT_CSV_PATH',
    os.path.abspath(os.path.join(AP_DIR, 'gnss_tca_llh.csv'))
)
TCA_PVT_CSV_ENABLE = os.environ.get(
    'POCKETSDR_TCA_PVT_CSV_ENABLE', '0'
).strip().lower() not in ('0', 'false', 'off', 'no', '')
...
if 'Windows' in env:
    ...
    LIBSDR = AP_DIR + '/../lib/linux_x86_64/libsdr.so'
...
# TCA-ready GNSS packet accessor added in sdr_pvt.c.
# It returns text:
#   TCA,<num_sats>,<obssec>
#   <pr_corr>,<prrate_corr>,<x>,<y>,<z>,<vx>,<vy>,<vz>,<clk_rate>,<elev>,<CN0>,<signalName>
#   END
try:
    libsdr.sdr_rcv_tca_stat.argtypes = (c_void_p,)
    libsdr.sdr_rcv_tca_stat.restype = c_char_p
    TCA_STAT_AVAILABLE = True
except AttributeError:
    TCA_STAT_AVAILABLE = False
...
tca_serial = None
tca_serial_rx_buf = ''
tca_last_obssec = None
tca_last_tx_time = 0.0
tca_page = None

# First-sample GNSS/IMU synchronization state. The first valid TCA packet is
# held in RAM until the IMU simulator confirms it selected the matching
# trajectory sample. This state is used only when all sync files are provided.
tca_sync_epoch_published = False
tca_sync_go_released = False
tca_sync_pending_packet = None
tca_sync_pending_obssec = None
tca_outage_start_time = None
tca_last_outage_active = False

# Latest processed TCA/INS solution returned by Teensy on Serial1.
# Teensy format is kept exactly as requested:
#   TCA,time,lat_deg,lon_deg,h_m,vn,ve,vd,roll,pitch,yaw,0.0000,0.0000
tca_last_tca_pos = None       # [lat_rad, lon_rad, alt_m]
tca_last_tca_vel = None       # [vn_mps, ve_mps, vd_mps]
tca_last_tca_att = None       # [roll_deg, pitch_deg, yaw_deg]
tca_last_tca_time = None      # IMU_time field from Teensy
tca_last_tca_rx_wall = 0.0    # PC receive wall-clock time
tca_rx_line_count = 0
tca_rx_parse_count = 0
tca_rx_byte_count = 0
tca_last_raw_rx_line = '-'
tca_last_raw_rx_bytes = '-'
tca_last_rx_parse_status = '-'

# Latest PocketSDR solution display values. Receiver velocity is estimated from
# consecutive FIX positions because the current sdr_rcv_pvt_sol() string does
# not expose velocity fields.
tca_last_pocket_pos = None    # [lat_rad, lon_rad, alt_m]
tca_last_pocket_vel = None    # [vn_mps, ve_mps, vd_mps]

# Manual reference coordinate for TCA-vs-reference error calculation.
# Auto mode uses latest PocketSDR PVT. Manual mode uses this fixed LLH.
tca_manual_ref_pos = None     # [lat_rad, lon_rad, alt_m]
tca_last_ref_pos = None       # Latest reference position used by the error calculator
tca_last_ref_source = '-'     # 'PocketSDR latest FIX' or 'Manual coordinates'

tca_last_error_m = None
tca_last_height_error_m = None
tca_last_error_text = '-'
tca_last_height_error_text = '-'

# Latest real-time error between latest PocketSDR PVT and latest Teensy TCA/INS PVT.
# This is updated continuously for display and is independent of outage-history entries.
tca_realtime_h_error_m = None
tca_realtime_v_error_m = None
tca_realtime_3d_error_m = None
tca_realtime_error_text = '-'
tca_realtime_height_error_text = '-'
tca_realtime_3d_error_text = '-'

# Keep every outage-end/manual error result in the current GUI session.
# Each entry stores horizontal haversine error and vertical height error.
tca_outage_counter = 0
tca_current_outage_id = 0
tca_error_history = []


# CSV logging for GNSS PVT and processed TCA/INS PVT.
tca_pvt_csv_enabled = TCA_PVT_CSV_ENABLE
tca_pvt_csv_path = TCA_PVT_CSV_DEFAULT_PATH
tca_pvt_csv_file = None
tca_pvt_csv_writer = None
tca_pvt_csv_rows = 0
...
tca_status = {
    ...
    'requested_rows': '-',
    'last_obssec': '-',
    'last_mode': 'Full TCA packet',
    'last_selected_signals': '-',
    'last_reason': '-',
    'elapsed': 0.0,
    'remaining': TCA_OUTAGE_DURATION,
    'height_error': '-',
    'error_history': '-',
    'normal_requested': '20',
    'reference_mode': 'Auto PocketSDR',
    'reference_source': '-',
    'reference_latest': '-',
    'reference_rate': '10 Hz',
    'manual_ref': '-',
    'realtime_error': '-',
    'realtime_height_error': '-',
    'realtime_3d_error': '-',
    'csv_status': 'ON' if tca_pvt_csv_enabled else 'OFF',
    'csv_file': tca_pvt_csv_path,
    'csv_rows': '0',

    # Serial RX/debug from Teensy
    'rx_bytes': '0',
    'rx_lines': '0',
    'rx_parsed': '0',
    'rx_status': '-',
    ...
    # Teensy processed TCA/INS solution
    'tca_time': '-',
    'tca_lat': '-',
    'tca_lon': '-',
    'tca_hgt': '-',
...
def rcv_open_sdev(sys_opt, inp_opt, out_opt, sig_opt, array_opt):
    sigs, prns = get_sig_opt(sig_opt)
    fmt = inp_opt.fmt_ids.get(inp_opt.fmt.get(), 1)
...
def rcv_open_file(sys_opt, inp_opt, out_opt, sig_opt, array_opt):
    sigs, prns = get_sig_opt(sig_opt)
    path_text = inp_opt.str_path.get().strip()
    if not path_text:
        return None, 'IF data file path is empty'
    path = os.path.abspath(os.path.expanduser(path_text))
    if not os.path.isfile(path):
        return None, 'IF data file not found: %s' % path
    file_size = os.path.getsize(path)
    if file_size == 0:
        return None, 'IF data file is empty: %s' % path
    fmt = inp_opt.fmt_ids.get(inp_opt.fmt.get(), 1)
    fs = to_float(inp_opt.fs.get()) * 1e6
    if fs <= 0.0:
        return None, 'Invalid IF sampling rate: %s Msps' % inp_opt.fs.get()
    ...
    info = '(file=%s, format=%s, fs=%.3fMsps, LO=%.4fMHz, size=%.1fMB)' % (
        path, inp_opt.fmt.get(), fs * 1e-6, fo[0] * 1e-6, file_size / 1e6)
    opt = rcv_opt_str(sys_opt, inp_opt, sig_opt, array_opt)
...
# get TCA-ready GNSS packet generated inside pntpos.c --------------------------
def get_tca_stat(rcv):
    if not rcv or not TCA_STAT_AVAILABLE:
        return ''
    try:
        result = libsdr.sdr_rcv_tca_stat(rcv)
        return result.decode(errors='ignore') if result else ''
    except Exception:
        return ''

# normalize/validate TCA packet before serial transmission ---------------------
def normalize_tca_packet(packet):
    if not packet:
        return None, None

    lines = [s.strip() for s in packet.splitlines() if s.strip()]
    if len(lines) < 3 or not lines[0].startswith('TCA,'):
        return None, None
    ...
    try:
        num_sats = int(fields[1])
        obssec = float(fields[2])
    except ValueError:
        return None, None
    ...
    row_lines = lines[1:end_idx] if end_idx is not None else lines[1:]
    if len(row_lines) < num_sats:
        return None, None

    # Send exactly the number of rows declared in the header.
    row_lines = row_lines[:num_sats]
    out_lines = [f'TCA,{num_sats},{obssec:.6f}'] + row_lines + ['END']
    return '\n'.join(out_lines) + '\n', obssec


# extract only TCA packet obssec without parsing all satellite rows -------------
def tca_packet_obssec(packet):
    if not packet:
        return None
    try:
        line = packet.splitlines()[0].strip()
        if not line.startswith('TCA,'):
            return None
...
# parse TCA packet rows for manual outage selection ----------------------------
def parse_tca_packet(packet):
    if not packet:
        return None, None, []

    lines = [s.strip() for s in packet.splitlines() if s.strip()]
    if len(lines) < 2 or not lines[0].startswith('TCA,'):
        return None, None, []
    ...
    rows = []
    for line in row_lines[:max(0, num_sats)]:
        ...
        try:
            rows.append({
                'line': line,
                'pr_corr': float(f[0]),
                'prrate_corr': float(f[1]),
                'sat_pos': np.array([float(f[2]), float(f[3]), float(f[4])], dtype=float),
                'sat_vel': np.array([float(f[5]), float(f[6]), float(f[7])], dtype=float),
                'clk_rate': float(f[8]),
                'elev': float(f[9]),
                'cn0': float(f[10]),
                'signal': f[11]
            ...
            continue
    return num_sats, obssec, rows

# build TCA packet from selected rows -----------------------------------------
def build_tca_packet(obssec, rows):
    out_lines = ['TCA,%d,%.6f' % (len(rows), obssec)]
    out_lines += [r['line'] if isinstance(r, dict) else str(r) for r in rows]
    out_lines.append('END')
    return '\n'.join(out_lines) + '\n'

# latest PocketSDR PVT receiver ECEF position from solution log ----------------
def latest_pocket_ecef():
    if len(sol_log) <= 0:
        return None
    try:
        return np.asarray(sdr_rtk.pos2ecef(sol_log[-1][1]), dtype=float)
    except Exception:
        return None

# redundancy-matrix satellite subset selection --------------------------------
def select_tca_rows_redundancy(rows, intended_sat_num):
    n = len(rows)
    intended_sat_num = int(np.clip(intended_sat_num, 0, 20))
    if intended_sat_num <= 0:
        return []
    if n <= intended_sat_num:
        return rows[:]

    rec_pos = latest_pocket_ecef()
    if rec_pos is None or not np.all(np.isfinite(rec_pos)):
        # Safe fallback when no receiver reference position is available.
        # This still enforces the requested outage count without heavy geometry.
        return sorted(rows, key=lambda r: r.get('cn0', 0.0), reverse=True)[:intended_sat_num]

    sat_pos = np.asarray([r['sat_pos'] for r in rows], dtype=float)
    los = sat_pos - rec_pos.reshape(1, 3)
    norm_los = np.linalg.norm(los, axis=1)
    good = np.isfinite(norm_los) & (norm_los > 0.0)
    if np.sum(good) < intended_sat_num:
        return sorted(rows, key=lambda r: r.get('cn0', 0.0), reverse=True)[:intended_sat_num]

    # Keep only valid LOS rows during selection, then map back to original rows.
    valid_indices = [i for i, ok in enumerate(good) if ok]
    valid_rows = [rows[i] for i in valid_indices]
    los = los[good]
    unit_los = los / np.linalg.norm(los, axis=1).reshape(-1, 1)
    m = len(valid_rows)

    matrix_d = np.eye(m)
    for r in range(1, m):
        for c in range(r):
            matrix_d[r, c] = float(np.dot(unit_los[r], unit_los[c]))
    matrix_redundancy = matrix_d * matrix_d
    matrix_redundancy = matrix_redundancy + np.tril(matrix_redundancy, -1).T

    eliminate = np.zeros(m, dtype=bool)
    while int(np.sum(eliminate)) < m - intended_sat_num:
        redundancy = np.sum(matrix_redundancy, axis=0)
        flag = int(np.argmax(redundancy))
        eliminate[flag] = True
        matrix_redundancy[flag, :] = 0.0
        matrix_redundancy[:, flag] = 0.0

    selected = [valid_rows[i] for i in range(m) if not eliminate[i]]
    if len(selected) < intended_sat_num:
        selected_set = set(id(r) for r in selected)
        for r in sorted(rows, key=lambda x: x.get('cn0', 0.0), reverse=True):
            if id(r) not in selected_set:
                selected.append(r)
                selected_set.add(id(r))
            if len(selected) >= intended_sat_num:
                break
    return selected[:intended_sat_num]

# haversine horizontal distance in meters --------------------------------------
...
# selected reference mode for TCA error calculation -----------------------------
def tca_error_ref_mode():
    if tca_page is not None and hasattr(tca_page, 'error_ref_mode'):
        mode = str(tca_page.error_ref_mode.get()).strip()
        return 'Manual' if mode.startswith('Manual') else 'Auto'
    return 'Auto'


# format LLH reference for GUI --------------------------------------------------
def format_llh(pos):
    if pos is None:
        return '-'
    try:
        return '%.8f, %.8f, %.3f m' % (pos[0] / D2R, pos[1] / D2R, pos[2])
    except Exception:
        return '-'


# get reference position for TCA error calculation -----------------------------
def get_tca_error_reference(use_latest_pocket=True, tca_time=None):
    global tca_last_ref_pos, tca_last_ref_source

    mode = tca_error_ref_mode()
    if mode == 'Manual':
        tca_status['reference_mode'] = 'Manual coordinates'
        if tca_manual_ref_pos is None:
            tca_last_ref_pos = None
            tca_last_ref_source = 'Manual reference not set'
            tca_status['reference_source'] = tca_last_ref_source
            tca_status['reference_latest'] = '-'
            return None, tca_last_ref_source
        tca_last_ref_pos = tca_manual_ref_pos
        tca_last_ref_source = 'Manual coordinates'
        tca_status['reference_source'] = tca_last_ref_source
        tca_status['reference_latest'] = format_llh(tca_manual_ref_pos)
        tca_status['manual_ref'] = format_llh(tca_manual_ref_pos)
        return tca_manual_ref_pos, tca_last_ref_source

    tca_status['reference_mode'] = 'Auto PocketSDR'
    if len(sol_log) == 0:
        tca_last_ref_pos = None
        tca_last_ref_source = 'Waiting for PocketSDR FIX'
        tca_status['reference_source'] = tca_last_ref_source
        tca_status['reference_latest'] = '-'
        return None, tca_last_ref_source
    ...
    try:
        if tca_time is not None:
            best_idx = None
            best_dt = None
            for idx, entry in enumerate(sol_log):
                try:
                    pocket_time = float(entry[0])
                    dt = abs(sdr_rtk.timediff(float(tca_time), pocket_time))
                except Exception:
                    continue
            ...
            if best_idx is not None:
                ref_pos = sol_log[best_idx][1]
                tca_last_ref_pos = ref_pos
                tca_last_ref_source = 'PocketSDR nearest FIX'
                tca_status['reference_source'] = tca_last_ref_source
                tca_status['reference_latest'] = format_llh(ref_pos)
                return ref_pos, tca_last_ref_source

        idx = -1 if use_latest_pocket else -2
        if len(sol_log) < abs(idx):
            tca_last_ref_pos = None
            tca_last_ref_source = 'Waiting for PocketSDR FIX'
            tca_status['reference_source'] = tca_last_ref_source
            tca_status['reference_latest'] = '-'
        ...
        ref_pos = sol_log[idx][1]
        tca_last_ref_pos = ref_pos
        tca_last_ref_source = 'PocketSDR latest FIX' if idx == -1 else 'PocketSDR 2nd-last FIX'
        tca_status['reference_source'] = tca_last_ref_source
        tca_status['reference_latest'] = format_llh(ref_pos)
        return ref_pos, tca_last_ref_source
    except Exception:
        tca_last_ref_pos = None
        tca_last_ref_source = 'PocketSDR reference unavailable'
        tca_status['reference_source'] = tca_last_ref_source
        tca_status['reference_latest'] = '-'
        return None, tca_last_ref_source


# update displayed manual reference --------------------------------------------
def update_tca_reference_display():
    tca_status['manual_ref'] = format_llh(tca_manual_ref_pos)
    if tca_error_ref_mode() == 'Manual' and tca_manual_ref_pos is not None:
        tca_status['reference_latest'] = format_llh(tca_manual_ref_pos)
        tca_status['reference_source'] = 'Manual coordinates'
        tca_status['reference_mode'] = 'Manual coordinates'
    elif tca_error_ref_mode() == 'Auto':
        tca_status['reference_mode'] = 'Auto PocketSDR'
        if len(sol_log) > 0:
            tca_status['reference_latest'] = format_llh(sol_log[-1][1])
            tca_status['reference_source'] = 'PocketSDR latest FIX'


# format GPST object for CSV ----------------------------------------------------
def tca_pvt_csv_gpst_str(t):
    try:
        ep = sdr_rtk.time2epoch(t)
        sec = float(ep[5])
        return '%04d-%02d-%02d %02d:%02d:%06.3f' % (
            int(ep[0]), int(ep[1]), int(ep[2]), int(ep[3]), int(ep[4]), sec)
    except Exception:
        return ''

# close TCA/GNSS PVT CSV file --------------------------------------------------
def tca_pvt_csv_close():
    global tca_pvt_csv_file, tca_pvt_csv_writer
    if tca_pvt_csv_file is not None:
        try:
            tca_pvt_csv_file.flush()
            tca_pvt_csv_file.close()
        except Exception:
            pass
    tca_pvt_csv_file = None
    tca_pvt_csv_writer = None

# open TCA/GNSS PVT CSV file ---------------------------------------------------
def tca_pvt_csv_open():
    global tca_pvt_csv_file, tca_pvt_csv_writer

    if not tca_pvt_csv_enabled:
        return False
    if tca_pvt_csv_file is not None and not tca_pvt_csv_file.closed:
        return True

    path = str(tca_pvt_csv_path).strip()
    if not path:
        tca_status['csv_status'] = 'CSV path not set'
        return False
    ...
    try:
        ...
        new_file = (not os.path.exists(abs_path)) or os.path.getsize(abs_path) == 0
        tca_pvt_csv_file = open(abs_path, 'a', newline='', buffering=1)
        tca_pvt_csv_writer = csv.writer(tca_pvt_csv_file)
        if new_file:
            tca_pvt_csv_writer.writerow(TCA_PVT_CSV_HEADER)
        tca_status['csv_status'] = 'ON'
        tca_status['csv_file'] = abs_path
        return True
    except Exception as e:
        tca_status['csv_status'] = 'CSV open error: %s' % str(e)
        tca_pvt_csv_close()
        return False

# write one GNSS/TCA LLH CSV row ---------------------------------------------
def tca_pvt_csv_write(gnss_time, gnss_pos, nsat):
    global tca_pvt_csv_rows

    if not tca_pvt_csv_enabled:
        return
    if not tca_pvt_csv_open():
        return

    try:
        wall = time.time()
        gnss_lat_deg = gnss_pos[0] / D2R
        gnss_lon_deg = gnss_pos[1] / D2R
        gnss_h_m = gnss_pos[2]

        tca_time = tca_lat_deg = tca_lon_deg = tca_h_m = ''
        if tca_last_tca_pos is not None:
            tca_lat_deg = tca_last_tca_pos[0] / D2R
            tca_lon_deg = tca_last_tca_pos[1] / D2R
            tca_h_m = tca_last_tca_pos[2]
        if tca_last_tca_time is not None:
            tca_time = tca_last_tca_time

        tca_pvt_csv_writer.writerow([
            '%.3f' % wall,
            time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(wall)),
            tca_pvt_csv_gpst_str(gnss_time),
            '%.10f' % gnss_lat_deg,
            '%.10f' % gnss_lon_deg,
            '%.4f' % gnss_h_m,
            '' if tca_time == '' else '%.6f' % float(tca_time),
            '' if tca_lat_deg == '' else '%.10f' % float(tca_lat_deg),
            '' if tca_lon_deg == '' else '%.10f' % float(tca_lon_deg),
            '' if tca_h_m == '' else '%.4f' % float(tca_h_m),
        ])
        tca_pvt_csv_rows += 1
        tca_status['csv_rows'] = str(tca_pvt_csv_rows)
        tca_status['csv_status'] = 'ON'
    except Exception as e:
        tca_status['csv_status'] = 'CSV write error: %s' % str(e)
        tca_pvt_csv_close()


# update PocketSDR position/velocity display for TCA tab -----------------------
def update_tca_pocket_solution_display():
    global tca_last_pocket_pos, tca_last_pocket_vel

    try:
        sol = parse_rcv_pvt_solution(get_rcv_pvt_sol(rcv_body))
        if sol is not None and sol[3] == 'FIX':
            tca_last_pocket_pos = sol[1]
            pos = tca_last_pocket_pos
            tca_status['pocket_latest'] = '%.8f, %.8f, %.3f m' % (
                pos[0] / D2R, pos[1] / D2R, pos[2])
        elif len(sol_log) > 0:
            tca_last_pocket_pos = sol_log[-1][1]
            pos = tca_last_pocket_pos
            tca_status['pocket_latest'] = '%.8f, %.8f, %.3f m' % (
                pos[0] / D2R, pos[1] / D2R, pos[2])
        else:
            tca_status['pocket_latest'] = '-'
            tca_status['pocket_vn'] = '-'
            tca_status['pocket_ve'] = '-'
            tca_status['pocket_vd'] = '-'
            return
    except Exception:
        if len(sol_log) <= 0:
            tca_status['pocket_latest'] = '-'
    ...
    if len(sol_log) >= 2:
        try:
            t0, p0 = sol_log[-2][0], sol_log[-2][1]
            t1, p1 = sol_log[-1][0], sol_log[-1][1]
            dt = sdr_rtk.timediff(t1, t0)
            if dt > 1e-3:
                ecef0 = np.asarray(sdr_rtk.pos2ecef(p0), dtype=float)
                ecef1 = np.asarray(sdr_rtk.pos2ecef(p1), dtype=float)
                enu = np.asarray(sdr_rtk.ecef2enu(p0, ecef1 - ecef0), dtype=float)
                # ecef2enu returns [east, north, up]. TCA display requires NED.
                vn = enu[1] / dt
                ve = enu[0] / dt
                vd = -enu[2] / dt
                tca_last_pocket_vel = [vn, ve, vd]
                tca_status['pocket_vn'] = '%.3f' % vn
                tca_status['pocket_ve'] = '%.3f' % ve
                tca_status['pocket_vd'] = '%.3f' % vd
        except Exception:
            pass
    ...
    if len(sol_log) >= 2:
        try:
            pos = sol_log[-2][1]
            tca_status['pocket_ref'] = '%.8f, %.8f, %.3f m' % (
                pos[0] / D2R, pos[1] / D2R, pos[2])
        except Exception:
            tca_status['pocket_ref'] = '-'


# parse processed TCA/INS solution returned by Teensy --------------------------
def parse_tca_return_line(line):
    global tca_last_tca_pos, tca_last_tca_vel, tca_last_tca_att
    global tca_last_tca_time, tca_last_tca_rx_wall
    global tca_rx_line_count, tca_rx_parse_count
    global tca_last_raw_rx_line, tca_last_rx_parse_status
    ...
    tca_rx_line_count += 1
    tca_last_raw_rx_line = line[:240]
    tca_status['rx_lines'] = str(tca_rx_line_count)
    tca_status['rx_last_line'] = tca_last_raw_rx_line

    fields = [s.strip() for s in line.split(',')]

    # Keep exactly the user-provided Teensy prefix and field order:
    # TCA,time,lat_deg,lon_deg,h_m,vn,ve,vd,roll,pitch,yaw,0.0000,0.0000
    # Do not treat PocketSDR's own outbound packet header TCA,num_sats,obssec
    # as a returned solution; it has only 3 fields and is not read from RX here.
    if len(fields) < 11 or fields[0] != 'TCA':
        tca_last_rx_parse_status = 'Ignored non-solution line'
        tca_status['rx_status'] = tca_last_rx_parse_status
        if TCA_SERIAL_RETURN_DEBUG:
            print('TCA RX ignored:', line)
        return False

    try:
        imu_time = float(fields[1])
        lat_deg  = float(fields[2])
        lon_deg  = float(fields[3])
        hgt_m    = float(fields[4])
        vn       = float(fields[5])
        ve       = float(fields[6])
        vd       = float(fields[7])
        roll     = float(fields[8])
        ...
        tca_last_rx_parse_status = 'Parse error: numeric field invalid'
        tca_status['rx_status'] = tca_last_rx_parse_status
        if TCA_SERIAL_RETURN_DEBUG:
            print('TCA RX parse error:', line)
        return False

    tca_last_tca_time = imu_time
    tca_last_tca_pos = [lat_deg * D2R, lon_deg * D2R, hgt_m]
    tca_last_tca_vel = [vn, ve, vd]
    tca_last_tca_att = [roll, pitch, yaw]
    tca_last_tca_rx_wall = time.time()
    tca_rx_parse_count += 1

    tca_status['rx_parsed'] = str(tca_rx_parse_count)
    tca_last_rx_parse_status = 'OK'
    tca_status['rx_status'] = tca_last_rx_parse_status
    tca_status['tca_time'] = '%.3f' % imu_time
    tca_status['tca_lat'] = '%.7f' % lat_deg
    tca_status['tca_lon'] = '%.7f' % lon_deg
    tca_status['tca_hgt'] = '%.3f' % hgt_m
    tca_status['tca_vn'] = '%.3f' % vn
    tca_status['tca_ve'] = '%.3f' % ve
    tca_status['tca_vd'] = '%.3f' % vd
    tca_status['tca_roll'] = '%.3f' % roll
    tca_status['tca_pitch'] = '%.3f' % pitch
    tca_status['tca_yaw'] = '%.3f' % yaw
    tca_status['last_reason'] = 'TCA processed solution received from Teensy'

    if TCA_SERIAL_RETURN_DEBUG and (tca_rx_parse_count <= 5 or tca_rx_parse_count % 200 == 0):
        print('TCA RX parsed #%d:' % tca_rx_parse_count, line)

    return True


# drain high-rate Teensy serial RX without redrawing GUI -------------
def read_tca_serial_rx():
    global tca_serial_rx_buf
    global tca_rx_byte_count, tca_last_raw_rx_bytes

    if not TCA_SERIAL_ENABLE:
        return
    if not tca_serial_open():
        return

    try:
        n = getattr(tca_serial, 'in_waiting', 0)
        if n <= 0:
            return

        n = min(n, TCA_SERIAL_RX_MAX_BYTES)
        data = tca_serial.read(n)
        if not data:
            return

        tca_rx_byte_count += len(data)
        # Show byte-level RX even if there is no newline yet. This is the key
        # diagnostic for 200 Hz Teensy streams.
        tail = data[-160:]
        try:
            preview = tail.decode('ascii', errors='replace')
        except Exception:
            preview = repr(tail)
        preview = preview.replace('\r', '\\r').replace('\n', '\\n')
        tca_last_raw_rx_bytes = preview
        tca_status['rx_bytes'] = str(tca_rx_byte_count)
        tca_status['rx_last_bytes'] = preview

        if TCA_SERIAL_RETURN_DEBUG and (tca_rx_byte_count <= 1000 or tca_rx_byte_count % 20000 < len(data)):
            print('TCA RX bytes total=%d chunk=%d tail=%s' %
                  (tca_rx_byte_count, len(data), preview))

        text = data.decode('ascii', errors='ignore')
        # Accept both LF and CR line endings. Teensy println()/printf("...\\n")
        # normally sends LF, but this makes parser robust.
        text = text.replace('\r', '\n')
        tca_serial_rx_buf += text

        # Prevent unlimited growth if a newline is missing or data is corrupted.
        if len(tca_serial_rx_buf) > 65536:
            tca_serial_rx_buf = tca_serial_rx_buf[-4096:]
            tca_status['rx_status'] = 'RX buffer trimmed; no newline or corrupted stream'

        # At 200 Hz, many lines can arrive between GUI updates. Parse all
        # complete lines and keep the latest solution values in tca_status.
        lines = tca_serial_rx_buf.split('\n')
        tca_serial_rx_buf = lines[-1]
        for line in lines[:-1]:
            line = line.strip()
            if line:
                parse_tca_return_line(line)

    except Exception as e:
        print('TCA serial read error: %s' % (str(e)))
        tca_serial_close()



# calculate real-time selected-reference/TCA PVT error --------------------------
def calculate_tca_realtime_error():
    global tca_realtime_h_error_m, tca_realtime_v_error_m, tca_realtime_3d_error_m
    global tca_realtime_error_text, tca_realtime_height_error_text, tca_realtime_3d_error_text

    update_tca_pocket_solution_display()
    update_tca_reference_display()

    if tca_last_tca_pos is None:
        tca_realtime_h_error_m = None
        tca_realtime_v_error_m = None
        tca_realtime_3d_error_m = None
        tca_realtime_error_text = '-'
        tca_realtime_height_error_text = '-'
        tca_realtime_3d_error_text = '-'
        tca_status['realtime_error'] = '-'
        tca_status['realtime_height_error'] = '-'
        tca_status['realtime_3d_error'] = '-'
        tca_status['last_reason'] = 'No processed TCA/INS position for error calculation'
        return None

    ref_pos, ref_source = get_tca_error_reference(use_latest_pocket=True, tca_time=tca_last_tca_time)
    if ref_pos is None:
        tca_realtime_h_error_m = None
        tca_realtime_v_error_m = None
        tca_realtime_3d_error_m = None
        ...
        tca_status['realtime_height_error'] = '-'
        tca_status['realtime_3d_error'] = '-'
        tca_status['last_reason'] = ref_source
        return None

    try:
        # Auto mode: latest PocketSDR FIX vs latest TCA PVT.
        # Manual mode: fixed manual LLH vs latest TCA PVT.
        # This function runs through the TCA page update loop at UD_CYCLE2=100 ms,
        # so both Auto and Manual comparisons update at 10 Hz.
        horizontal_m = haversine_m(ref_pos, tca_last_tca_pos)
        vertical_m = tca_last_tca_pos[2] - ref_pos[2]  # signed: TCA height - reference height
        err3d_m = sqrt(horizontal_m * horizontal_m + vertical_m * vertical_m)

        tca_realtime_h_error_m = horizontal_m
        tca_realtime_v_error_m = vertical_m
        tca_realtime_3d_error_m = err3d_m

        tca_realtime_error_text = '%.3f m' % horizontal_m
        tca_realtime_height_error_text = '%+.3f m  (|V| %.3f m)' % (
            vertical_m, abs(vertical_m))
        tca_realtime_3d_error_text = '%.3f m' % err3d_m

        tca_status['realtime_error'] = tca_realtime_error_text
        tca_status['realtime_height_error'] = tca_realtime_height_error_text
        tca_status['realtime_3d_error'] = tca_realtime_3d_error_text
        tca_status['reference_source'] = ref_source
        return horizontal_m, vertical_m, err3d_m
...
# format outage-error history for GUI ------------------------------------------
def format_tca_error_history(max_lines=None):
    if not tca_error_history:
        return '-'
    rows = tca_error_history if max_lines is None else tca_error_history[-max_lines:]
    return '\n'.join([
        '#%03d %s | %s | TCA %.3f | H %.3f m | V %.3f m | |V| %.3f m' % (
            e['id'], e['source'], e.get('ref_source', 'reference'), e['tca_time'],
            e['horizontal_m'], e['vertical_m'], abs(e['vertical_m'])) for e in rows
    ])


# calculate outage-end/manual horizontal + vertical errors ----------------------
def calculate_tca_outage_error(source='manual'):
    global tca_last_error_m, tca_last_height_error_m
    global tca_last_error_text, tca_last_height_error_text

    update_tca_pocket_solution_display()
    update_tca_reference_display()

    if tca_last_tca_pos is None:
        tca_status['last_reason'] = 'No processed TCA/INS position from Teensy for error calculation'
        return None

    # Auto outage/history previously used the 2nd-last PocketSDR FIX to avoid
    # using a not-yet-stabilized latest GUI sample. For auto-ended outages
    # (timer-based), prefer the latest PocketSDR realtime FIX so the outage-end
    # analysis reflects the most recent real-time solution. Manual mode keeps
    # using the fixed manual coordinate directly.
    use_latest = True if str(source).lower().startswith('auto') else False
    ref_pos, ref_source = get_tca_error_reference(use_latest_pocket=use_latest, tca_time=tca_last_tca_time)
    if ref_pos is None:
        tca_status['last_reason'] = ref_source
        return None
    ...
    try:
        ...
        tca_last_error_m = horizontal_m
        tca_last_height_error_m = vertical_m
        tca_last_error_text = '%.3f m' % horizontal_m
        tca_last_height_error_text = '%+.3f m  (|V| %.3f m)' % (vertical_m, abs(vertical_m))

        outage_id = tca_current_outage_id if tca_current_outage_id > 0 else len(tca_error_history) + 1
        try:
            tca_time = float(tca_last_tca_time) if tca_last_tca_time is not None else 0.0
        except Exception:
            tca_time = 0.0
        entry = {
            'id': int(outage_id),
            'source': str(source),
            'duration': float(tca_status.get('elapsed', 0.0)),
            'ref_source': str(ref_source),
            'tca_time': tca_time,
            'horizontal_m': float(horizontal_m),
            'vertical_m': float(vertical_m),
            'ref_lat_deg': float(ref_pos[0] / D2R),
            'ref_lon_deg': float(ref_pos[1] / D2R),
            'ref_h_m': float(ref_pos[2]),
            'tca_lat_deg': float(tca_last_tca_pos[0] / D2R),
            'tca_lon_deg': float(tca_last_tca_pos[1] / D2R),
            'tca_h_m': float(tca_last_tca_pos[2]),
            'wall_time': time.time(),
        }
        tca_error_history.append(entry)
        tca_status['height_error'] = tca_last_height_error_text
        tca_status['error_history'] = format_tca_error_history()
        tca_status['last_reason'] = 'Error saved vs %s: H %.3f m, V %+.3f m' % (
            ref_source, horizontal_m, vertical_m)
        return horizontal_m, vertical_m

    except Exception as e:
        tca_status['last_reason'] = 'Error calc failed: %s' % str(e)
        return None

# return whether manual outage is currently enabled ----------------------------
def tca_outage_enabled():
    return tca_page is not None and hasattr(tca_page, 'outage_enable') and \
        bool(tca_page.outage_enable.get())

# start/stop outage timer on checkbox transition -------------------------------
def update_tca_outage_state():
    global tca_outage_start_time, tca_last_outage_active
    global tca_outage_counter, tca_current_outage_id

    ena = tca_outage_enabled()
    now = time.time()

    if ena and not tca_last_outage_active:
        tca_outage_counter += 1
        tca_current_outage_id = tca_outage_counter
        tca_outage_start_time = now
        tca_last_outage_active = True
        tca_status['last_reason'] = 'Outage #%03d started' % tca_current_outage_id

    if not ena and tca_last_outage_active:
        tca_last_outage_active = False
        tca_outage_start_time = None
        tca_status['elapsed'] = 0.0
        tca_status['remaining'] = TCA_OUTAGE_DURATION
        tca_status['last_reason'] = 'Outage disabled'

# auto-end outage after 20 seconds --------------------------------------------
def check_tca_outage_timeout():
    global tca_outage_start_time, tca_last_outage_active

    if not tca_outage_enabled():
        return False
    if tca_outage_start_time is None:
        tca_outage_start_time = time.time()
        return False

    elapsed = time.time() - tca_outage_start_time
    remaining = max(0.0, TCA_OUTAGE_DURATION - elapsed)
    tca_status['elapsed'] = elapsed
    tca_status['remaining'] = remaining

    if elapsed >= TCA_OUTAGE_DURATION:
        calculate_tca_outage_error(source='auto-end')
        if tca_page is not None and hasattr(tca_page, 'outage_enable'):
            tca_page.outage_enable.set(False)
        tca_last_outage_active = False
        tca_outage_start_time = None
        tca_status['elapsed'] = TCA_OUTAGE_DURATION
        tca_status['remaining'] = 0.0
        tca_status['last_reason'] = 'Outage auto-ended after %.0f s' % TCA_OUTAGE_DURATION
        return True
    return False


# apply normal-condition satellite-count filtering to TCA serial packet ---------
def apply_tca_normal_selection(packet):
    _, obssec, rows = parse_tca_packet(packet)
    if obssec is None:
        return None, None

    tca_status['available_rows'] = len(rows)
    tca_status['last_obssec'] = '%.3f' % obssec

    try:
        req = int(tca_page.normal_sat_count.get()) if tca_page is not None and \
            hasattr(tca_page, 'normal_sat_count') else 20
    except Exception:
        req = 20
    req = int(np.clip(req, 0, 20))

    selected = select_tca_rows_redundancy(rows, req)
    out_packet = build_tca_packet(obssec, selected)

    tca_status['selected_rows'] = len(selected)
    tca_status['requested_rows'] = str(req)
    tca_status['normal_requested'] = str(req)
    tca_status['last_mode'] = 'Normal / RedundancyMatrix' if req < len(rows) else 'Normal / Full available'
    tca_status['last_selected_signals'] = ', '.join([r['signal'] for r in selected[:8]]) if selected else '-'
    tca_status['last_reason'] = 'Normal TCA packet: %d/%d satellites' % (len(selected), len(rows))
    return out_packet, obssec


# apply manual outage filtering to TCA serial packet ---------------------------
def apply_tca_manual_outage(packet):
    update_tca_outage_state()
    ...
    # Timeout check happens before filtering. Once elapsed, normal-condition
    # satellite-count control resumes immediately.
    timed_out = check_tca_outage_timeout()
    if not tca_outage_enabled() or timed_out:
        return apply_tca_normal_selection(packet)

    try:
        req = int(tca_page.sat_count.get()) if tca_page is not None else 0
    except Exception:
        req = 0
    req = int(np.clip(req, 0, 3))
    ...
    tca_status['selected_rows'] = len(selected)
    tca_status['requested_rows'] = str(req)
    tca_status['last_mode'] = 'Manual outage / RedundancyMatrix'
    tca_status['last_selected_signals'] = ', '.join([r['signal'] for r in selected]) if selected else '-'
    tca_status['last_reason'] = 'Reduced TCA packet'
    return out_packet, obssec

# TCA/INS position manual input ------------------------------------------------
def on_tca_pos_push(e=None):
    global tca_last_tca_pos

    if tca_page is None:
        return
    label = ('TCA/INS Latitude (deg)', 'TCA/INS Longitude (deg)', 'TCA/INS Altitude (m)')
    dlg = sdr_opt.modal_dlg_new(root, 300, 160, 'TCA/INS Position')
    var = [StringVar() for _ in range(3)]
    if tca_last_tca_pos is not None:
        var[0].set('%.8f' % (tca_last_tca_pos[0] / D2R))
        var[1].set('%.8f' % (tca_last_tca_pos[1] / D2R))
        var[2].set('%.3f' % (tca_last_tca_pos[2]))
    else:
        var[0].set('0.00000000')
        var[1].set('0.00000000')
        var[2].set('0.000')
    for i in range(3):
        sdr_opt.inp_panel_new(dlg.panel, label[i], var[i], width=14, pwidth=210)
    root.wait_window(dlg.win)
    if dlg.ok:
        lat = to_float(var[0].get()) * D2R
        lon = to_float(var[1].get()) * D2R
        alt = to_float(var[2].get())
        tca_last_tca_pos = [lat, lon, alt]
        tca_status['last_reason'] = 'TCA/INS position updated'
        update_tca_page(tca_page)


# Manual reference position input ---------------------------------------------
def on_tca_ref_pos_push(e=None):
    global tca_manual_ref_pos

    if tca_page is None:
        return
    label = ('Reference Latitude (deg)', 'Reference Longitude (deg)', 'Reference Altitude (m)')
    dlg = sdr_opt.modal_dlg_new(root, 330, 165, 'Manual Error Reference Position')
    var = [StringVar() for _ in range(3)]
    if tca_manual_ref_pos is not None:
        var[0].set('%.10f' % (tca_manual_ref_pos[0] / D2R))
        var[1].set('%.10f' % (tca_manual_ref_pos[1] / D2R))
        var[2].set('%.3f' % (tca_manual_ref_pos[2]))
    elif len(sol_log) > 0:
        # Convenience: initialize with latest PocketSDR position.
        pos = sol_log[-1][1]
        var[0].set('%.10f' % (pos[0] / D2R))
        var[1].set('%.10f' % (pos[1] / D2R))
        var[2].set('%.3f' % (pos[2]))
    else:
        var[0].set('0.0000000000')
        var[1].set('0.0000000000')
        var[2].set('0.000')
    for i in range(3):
        sdr_opt.inp_panel_new(dlg.panel, label[i], var[i], width=16, pwidth=220)
    root.wait_window(dlg.win)
    if dlg.ok:
        lat_deg = to_float(var[0].get())
        lon_deg = to_float(var[1].get())
        alt = to_float(var[2].get())
        tca_manual_ref_pos = [lat_deg * D2R, lon_deg * D2R, alt]
        if hasattr(tca_page, 'error_ref_mode'):
            tca_page.error_ref_mode.set('Manual')
        tca_status['manual_ref'] = format_llh(tca_manual_ref_pos)
        tca_status['last_reason'] = 'Manual reference position updated'
        update_tca_page(tca_page)


# TCA outage checkbox callback -------------------------------------------------
def on_tca_outage_toggle():
    global tca_outage_start_time, tca_last_outage_active
    global tca_outage_counter, tca_current_outage_id

    if tca_page is None:
        return
    tca_page.outage_enable.set(not tca_page.outage_enable.get())
    if tca_outage_enabled():
        tca_outage_counter += 1
        tca_current_outage_id = tca_outage_counter
        tca_outage_start_time = time.time()
        tca_last_outage_active = True
        ...
        if tca_last_outage_active:
            tca_status['elapsed'] = time.time() - tca_outage_start_time \
                if tca_outage_start_time is not None else 0.0
            calculate_tca_outage_error(source='manual-stop')
        tca_last_outage_active = False
        tca_outage_start_time = None
        tca_status['last_reason'] = 'Outage stopped manually'
    if tca_page is not None:
        update_tca_page(tca_page)

# manual error calculation button ---------------------------------------------
def on_tca_calc_error_push(e=None):
    calculate_tca_outage_error(source='manual-calc')
    if tca_page is not None:
        update_tca_page(tca_page)

# clear TCA error display ------------------------------------------------------
def on_tca_clear_error_push(e=None):
    global tca_last_error_m, tca_last_height_error_m
    global tca_last_error_text, tca_last_height_error_text
    global tca_error_history
    tca_last_error_m = None
    tca_last_height_error_m = None
    tca_last_error_text = '-'
    tca_last_height_error_text = '-'
    tca_error_history.clear()
    tca_status['height_error'] = '-'
    tca_status['error_history'] = '-'
    if tca_page is not None:
        update_tca_page(tca_page)

# open serial port for TCA packet TX/RX ---------------------------------------
def tca_serial_open():
    global tca_serial
    ...
    if serial is None:
        print('TCA serial disabled: pyserial is not installed.')
        return False

    if tca_serial is not None and getattr(tca_serial, 'is_open', False):
        return True

    if not TCA_SERIAL_PORT:
        print('TCA serial disabled: POCKETSDR_TCA_PORT is not set.')
        return False

    try:
        tca_serial = serial.Serial(
            TCA_SERIAL_PORT,
            TCA_SERIAL_BAUDRATE,
            timeout=0,
            write_timeout=TCA_SERIAL_WRITE_TIMEOUT,
            rtscts=False,
            dsrdtr=False,
            xonxoff=False
        )
        try:
            # On first open only: remove stale bytes from a previous run.
            tca_serial.reset_input_buffer()
            tca_serial.reset_output_buffer()
        except Exception:
            pass
        time.sleep(0.1)
        print('TCA serial opened: %s @ %d bps' %
              (TCA_SERIAL_PORT, TCA_SERIAL_BAUDRATE))
        return True
    except Exception as e:
        tca_serial = None
        print('TCA serial open error: %s' % (str(e)))
        return False

# close serial port for TCA packet transmission -------------------------------
def tca_serial_close():
    global tca_serial

    if tca_serial is None:
        return

    try:
        if getattr(tca_serial, 'is_open', False):
            tca_serial.close()
    except Exception:
        pass
    tca_serial = None

# transmit TCA packet once per new GNSS epoch ----------------------------------
def _tca_first_sample_sync_enabled():
    return bool(
        POCKETSDR_AUTOSTART and
        POCKETSDR_SIM_EPOCH_FILE and
        POCKETSDR_SIM_ALIGNED_FILE and
        POCKETSDR_SIM_GO_FILE
    )


def _write_tca_packet_now(packet, obssec):
    """Write one already-built TCA packet and update TX bookkeeping."""
    global tca_last_obssec, tca_last_tx_time

    if not packet or obssec is None:
        return False
    if not tca_serial_open():
        return False

    try:
        n = tca_serial.write(packet.encode('ascii', errors='ignore'))
        if n <= 0:
            return False
        tca_last_obssec = obssec
        tca_last_tx_time = time.time()
        return True
    except Exception as e:
        print('TCA serial write error: %s' % str(e))
        tca_serial_close()
        return False


def update_tca_serial():
    global tca_last_obssec, tca_last_tx_time
    global tca_sync_epoch_published, tca_sync_go_released
    global tca_sync_pending_packet, tca_sync_pending_obssec

    if not TCA_SERIAL_ENABLE or not rcv_body:
        return

    # RX has its own high-rate timer. Do not drain Teensy RX here because the
    # variable amount of RX work would add jitter immediately before GNSS TX.

    # Once the first GNSS packet has been held, do not keep rebuilding it.
    # Wait for the IMU simulator to confirm that its starting array index is
    # aligned to the published obssec.
    if _tca_first_sample_sync_enabled() and tca_sync_epoch_published and \
            not tca_sync_go_released:
        aligned_file = os.path.abspath(os.path.expanduser(
            POCKETSDR_SIM_ALIGNED_FILE))
        if not os.path.isfile(aligned_file):
            return

        # Common GO: create it immediately before queueing the held first GNSS
        # packet. The IMU process is already prepared and polls GO at 1 ms.
        _sim_sync_write_file(
            POCKETSDR_SIM_GO_FILE,
            'GO obssec=%.6f pid=%d wall_time=%.9f\n' % (
                tca_sync_pending_obssec, os.getpid(), time.time())
        )
        tca_sync_go_released = True
        print(
            'GNSS/IMU sync: GO released for first obssec %.6f' %
            tca_sync_pending_obssec
        )

        if _write_tca_packet_now(
                tca_sync_pending_packet, tca_sync_pending_obssec):
            print(
                'GNSS/IMU sync: first aligned TCA packet transmitted, '
                'obssec=%.6f' % tca_sync_pending_obssec
            )
            tca_sync_pending_packet = None
            tca_sync_pending_obssec = None
        return

    raw_packet = get_tca_stat(rcv_body)
    obssec = tca_packet_obssec(raw_packet)
    if obssec is None:
        return

    # Do not retransmit an already-sent GNSS epoch.
    if tca_last_obssec is not None and abs(obssec - tca_last_obssec) < 1e-6:
        return

    if tca_outage_enabled():
        packet, obssec = apply_tca_manual_outage(raw_packet)
    else:
        packet, obssec = apply_tca_normal_selection(raw_packet)

    if not packet:
        return

    now = time.time()
    if TCA_TX_MIN_INTERVAL > 0.0 and now - tca_last_tx_time < TCA_TX_MIN_INTERVAL:
        return

    if not tca_serial_open():
        return

    # In first-sample synchronization mode, HOLD the first valid packet.
    # Publish only its measurement epoch. The IMU process uses that epoch to
    # choose the matching precomputed accelerometer/gyro sample.
    if _tca_first_sample_sync_enabled() and not tca_sync_epoch_published:
        tca_sync_pending_packet = packet
        tca_sync_pending_obssec = obssec
        _sim_sync_write_file(
            POCKETSDR_SIM_EPOCH_FILE,
            '%.9f\n' % obssec
        )
        tca_sync_epoch_published = True
        print(
            'GNSS/IMU sync: first valid TCA packet held; '
            'published obssec=%.9f -> %s' %
            (obssec, POCKETSDR_SIM_EPOCH_FILE)
        )
        return

    _write_tca_packet_now(packet, obssec)
...
# parse a receiver PVT solution string ---------------------------------------
def parse_rcv_pvt_solution(sol_text):
    """Parse a receiver PVT solution string into time/position/status."""
    try:
        sol = str(sol_text).split()
    except Exception:
        return None
    if len(sol) < 7:
        return None
    ...
    if len(sol_log) >= 2:
        try:
            ...
            if dt > 1e-3:
                ...
                ecef1 = np.asarray(sdr_rtk.pos2ecef(p1), dtype=float)
                enu = np.asarray(sdr_rtk.ecef2enu(p0, ecef1 - ecef0), dtype=float)
                vel_str = '%.3f/%.3f/%.3f' % (enu[1] / dt, enu[0] / dt, -enu[2] / dt)
        except Exception:
            pass
    labels = labels[:3] + ('Vn/Ve/Vd (m/s)',) + labels[4:]
    val = stat[0:3] + [vel_str] + stat[3:10] + [sol[0] + ' ' + sol[1], sol[6]] + \
...
# generate TCA manual outage test page -----------------------------------------
def tca_page_new(parent):
    p = Obj()
    p.parent = parent
    p.panel = Frame(parent)
    p.toolbar = tool_bar_new(p.panel)

    p.outage_enable = BooleanVar(value=False)
    p.sat_count = StringVar(value='3')
    ttk.Label(p.toolbar, text='Running Algorithm: TCA').pack(side=LEFT, padx=(8, 18))
    ttk.Label(p.toolbar, text='GNSS Status:').pack(side=LEFT)
    p.gnss_status = ttk.Label(p.toolbar, text='●', foreground='#00A000')
    p.gnss_status.pack(side=LEFT, padx=(3, 25))
    ttk.Label(p.toolbar, text='Satellites to Outage:').pack(side=LEFT)
    p.box_sats = sel_box_new(p.toolbar, TCA_OUTAGE_SAT_OPTIONS, '3', width=3)
    p.box_sats.configure(textvariable=p.sat_count)
    p.box_sats.pack(side=LEFT, padx=(4, 22))
    ttk.Style().configure('TCA.TButton', foreground='white', background='#C52020')
    p.btn_outage = ttk.Button(p.toolbar, text='Manual Outage', style='TCA.TButton',
        command=on_tca_outage_toggle)
    p.btn_outage.pack(side=LEFT, padx=(0, 5))
    ttk.Label(p.toolbar, text='Duration:').pack(side=LEFT)
    ttk.Label(p.toolbar, text='20').pack(side=LEFT, padx=(4, 2))
    ttk.Label(p.toolbar, text='s').pack(side=LEFT)

    body = Frame(p.panel, bg=BG_COLOR1)
    body.pack(expand=1, fill=BOTH, padx=12, pady=6)

    live = Frame(body, relief=SOLID, bd=1, bg=BG_COLOR1)
    live.pack(fill=X, pady=(0, 6))
    ttk.Label(live, text='Live Coordinates', font=get_font(1, 'bold')).grid(
        row=0, column=0, columnspan=4, sticky=W, padx=8, pady=(4, 2))
    p.labels = {}
    for row, (name, key) in enumerate((
        ('Latitude:', 'latitude'), ('Longitude:', 'longitude'), ('Altitude:', 'altitude')),
        start=1):
        ttk.Label(live, text=name, font=get_font(1, 'bold')).grid(
            row=row, column=0, sticky=W, padx=(12, 4), pady=1)
        p.labels[key] = ttk.Label(live, text='-', foreground=P1_COLOR)
        p.labels[key].grid(row=row, column=1, sticky=W, padx=4, pady=1)
    for row, (name, key) in enumerate((
        ('vN:', 'vn'), ('vE:', 've'), ('vD:', 'vd')),
        start=1):
        ttk.Label(live, text=name, font=get_font(1, 'bold')).grid(
            row=row, column=2, sticky=W, padx=(150, 4), pady=1)
        p.labels[key] = ttk.Label(live, text='-', foreground=P1_COLOR)
        p.labels[key].grid(row=row, column=3, sticky=W, padx=4, pady=1)

    ttk.Label(body, text='Real-Time Error vs PVT', font=get_font(1, 'bold')).pack(
        anchor=W, padx=8, pady=(0, 2))
    charts = Frame(body, bg=BG_COLOR1)
    charts.pack(fill=X)
    p.plt_h = plt.plot_new(charts, 400, 175, title='Horizontal Error',
        xlabel='Time (TOW)', ylabel='Error (m)', margin=(45, 25, 25, 28))
    p.plt_v = plt.plot_new(charts, 400, 175, title='Vertical Error',
        xlabel='Time (TOW)', ylabel='Error (m)', margin=(45, 25, 25, 28))
    p.plt_h.c.pack(side=LEFT, expand=1, fill=BOTH, padx=(0, 8))
    p.plt_v.c.pack(side=LEFT, expand=1, fill=BOTH)
    # Keep the error view focused on the latest six seconds so startup spikes
    # do not determine the scale for the whole live plot.
    p.error_samples = deque(maxlen=60)

    attitude = Frame(body, relief=SOLID, bd=1, bg=BG_COLOR1)
    attitude.pack(fill=X, pady=(6, 5))
    ttk.Label(attitude, text='Real-Time TCA Attitude', font=get_font(1, 'bold')).grid(
        row=0, column=0, columnspan=6, sticky=W, padx=8, pady=(4, 2))
    for col, (name, key) in enumerate((
        ('Roll:', 'roll'), ('Pitch:', 'pitch'), ('Yaw:', 'yaw'))):
        ttk.Label(attitude, text=name, font=get_font(1, 'bold')).grid(
            row=1, column=col * 2, sticky=W, padx=(12 if col == 0 else 45, 4), pady=4)
        p.labels[key] = ttk.Label(attitude, text='-', foreground=P1_COLOR,
            font=get_font(2, 'bold'))
        p.labels[key].grid(row=1, column=col * 2 + 1, sticky=W, padx=4, pady=4)

    stats = Frame(body, relief=SOLID, bd=1, bg=BG_COLOR1)
    stats.pack(fill=BOTH, expand=1, pady=(6, 5))
    ttk.Label(stats, text='Statistics', font=get_font(1, 'bold')).pack(anchor=W, padx=8, pady=(3, 1))
    p.table = ttk.Treeview(stats, columns=('id', 'duration', 'horizontal', 'vertical'),
        show='headings', height=2)
    for key, title, width in (('id', 'Outage #', 95), ('duration', 'Duration', 110),
        ('horizontal', 'Horizontal', 115), ('vertical', 'Vertical', 115)):
        p.table.heading(key, text=title)
        p.table.column(key, width=width, anchor=W)
    p.table.pack(fill=BOTH, expand=1, padx=6, pady=(0, 5))

    summary = Frame(body, relief=SOLID, bd=1, bg=BG_COLOR1)
    summary.pack(fill=X, pady=(0, 5))
    ttk.Label(summary, text='Summary', font=get_font(1, 'bold')).pack(anchor=W, padx=8, pady=(3, 1))
    rms = Frame(summary, relief=SOLID, bd=1, bg='white')
    rms.pack(anchor=E, padx=350, pady=(0, 4))
    p.labels['horizontal_rms'] = ttk.Label(rms, text='--', width=11, anchor=CENTER)
    p.labels['vertical_rms'] = ttk.Label(rms, text='--', width=11, anchor=CENTER)
    for row, label, value in ((0, 'Horizontal RMS:', p.labels['horizontal_rms']),
        (1, 'Vertical RMS:', p.labels['vertical_rms'])):
        ttk.Label(rms, text=label, width=16, anchor=E).grid(row=row, column=0, padx=4)
        value.grid(row=row, column=1, padx=4)

    csv_frame = Frame(body, relief=SOLID, bd=1, bg=BG_COLOR1)
    csv_frame.pack(fill=X, pady=(0, 5))
    ttk.Label(csv_frame, text='GNSS/TCA LLH CSV Output', font=get_font(1, 'bold')).grid(
        row=0, column=0, columnspan=2, sticky=W, padx=8, pady=(3, 1))
    ttk.Label(csv_frame, text='Configured from Output ...', anchor=W).grid(
        row=1, column=0, sticky=W, padx=8, pady=1)
    p.labels['csv_status'] = ttk.Label(csv_frame, text='-', foreground=P1_COLOR)
    p.labels['csv_status'].grid(row=1, column=1, sticky=W, padx=8, pady=1)
    ttk.Label(csv_frame, text='CSV file', anchor=W).grid(row=2, column=0, sticky=W, padx=8, pady=1)
    p.labels['csv_file'] = ttk.Label(csv_frame, text='-', foreground=P1_COLOR, wraplength=620)
    p.labels['csv_file'].grid(row=2, column=1, sticky=W, padx=8, pady=1)
    ttk.Label(csv_frame, text='Rows', anchor=W).grid(row=3, column=0, sticky=W, padx=8, pady=(1, 4))
    p.labels['csv_rows'] = ttk.Label(csv_frame, text='0', foreground=P1_COLOR)
    p.labels['csv_rows'].grid(row=3, column=1, sticky=W, padx=8, pady=(1, 4))
    csv_frame.grid_columnconfigure(1, weight=1)

    update_tca_page(p)
    return p

# refresh TCA plots, statistics, and RMS values -------------------------------
def update_tca_error_display(p):
    now = tca_last_tca_time
    h_error = tca_realtime_h_error_m
    v_error = None if tca_realtime_v_error_m is None else abs(tca_realtime_v_error_m)
    if now is not None and h_error is not None and (not p.error_samples or p.error_samples[-1][0] != now):
        p.error_samples.append((float(now), float(h_error), float(v_error)))
    samples = list(p.error_samples)
    if samples:
        xs = [sample[0] for sample in samples]
        xlim = (min(xs), max(xs)) if min(xs) < max(xs) else (xs[0] - 1.0, xs[0] + 1.0)
        for plot, index, color in ((p.plt_h, 1, '#2878B5'), (p.plt_v, 2, '#C04040')):
            values = [sample[index] for sample in samples]
            ymax = max(1.0, max(values) * 1.15)
            plt.plot_xlim(plot, xlim)
            plt.plot_ylim(plot, (0.0, ymax))
            plt.plot_clear(plot)
            plt.plot_axis(plot)
            plt.plot_poly(plot, xs, values, color=color, width=2)
    else:
        for plot in (p.plt_h, p.plt_v):
            plt.plot_clear(plot)
            plt.plot_axis(plot)

    for item in p.table.get_children():
        p.table.delete(item)
    for entry in tca_error_history:
        p.table.insert('', END, values=(entry['id'], '%.0f s' % entry.get('duration', TCA_OUTAGE_DURATION),
            '%.3f m' % entry['horizontal_m'], '%.3f m' % abs(entry['vertical_m'])))
    if tca_error_history:
        horizontal = [entry['horizontal_m'] for entry in tca_error_history]
        vertical = [entry['vertical_m'] for entry in tca_error_history]
        p.labels['horizontal_rms'].configure(text='%.3f m' % sqrt(np.mean(np.square(horizontal))))
        p.labels['vertical_rms'].configure(text='%.3f m' % sqrt(np.mean(np.square(vertical))))
    else:
        p.labels['horizontal_rms'].configure(text='--')
        p.labels['vertical_rms'].configure(text='--')

# update TCA page --------------------------------------------------------------
def update_tca_page(p):
    if p is None:
        return

    update_tca_pocket_solution_display()
    update_tca_reference_display()
    calculate_tca_realtime_error()

    if tca_last_tca_rx_wall > 0.0:
        tca_status['tca_age'] = '%.3f s' % (time.time() - tca_last_tca_rx_wall)
    else:
        tca_status['tca_age'] = '-'

    if tca_outage_enabled() and tca_outage_start_time is not None:
        elapsed = time.time() - tca_outage_start_time
        remaining = max(0.0, TCA_OUTAGE_DURATION - elapsed)
        tca_status['elapsed'] = elapsed
        tca_status['remaining'] = remaining
    else:
        elapsed = tca_status.get('elapsed', 0.0)
        remaining = tca_status.get('remaining', TCA_OUTAGE_DURATION)

    state = 'ON' if tca_outage_enabled() else 'OFF'
    pos = tca_last_tca_pos
    vel = tca_last_tca_vel
    att = tca_last_tca_att
    if pos is None:
        for key in ('latitude', 'longitude', 'altitude', 'vn', 've', 'vd'):
            p.labels[key].configure(text='-')
        p.gnss_status.configure(foreground='#A0A0A0')
    else:
        p.labels['latitude'].configure(text='%.6f °' % (pos[0] / D2R))
        p.labels['longitude'].configure(text='%.6f °' % (pos[1] / D2R))
        p.labels['altitude'].configure(text='%.3f m' % pos[2])
        p.labels['vn'].configure(text='%.3f m/s' % vel[0] if vel else '-')
        p.labels['ve'].configure(text='%.3f m/s' % vel[1] if vel else '-')
        p.labels['vd'].configure(text='%.3f m/s' % vel[2] if vel else '-')
        p.gnss_status.configure(foreground='#00A000')

    if att is None:
        p.labels['roll'].configure(text='-')
        p.labels['pitch'].configure(text='-')
        p.labels['yaw'].configure(text='-')
    else:
        p.labels['roll'].configure(text='%+.3f °' % att[0])
        p.labels['pitch'].configure(text='%+.3f °' % att[1])
        p.labels['yaw'].configure(text='%+.3f °' % att[2])

    p.btn_outage.configure(text='Manual Outage' if not state == 'ON' else 'Manual Outage')
    if 'csv_status' in p.labels:
        p.labels['csv_status'].configure(text=tca_status.get('csv_status', '-'))
    if 'csv_file' in p.labels:
        p.labels['csv_file'].configure(text=tca_status.get('csv_file', '-'))
    if 'csv_rows' in p.labels:
        p.labels['csv_rows'].configure(text=tca_status.get('csv_rows', '0'))
    update_tca_error_display(p)
...
# GNSS/IMU synchronization helpers --------------------------------------------
def _sim_sync_write_file(path, text):
...
def reset_first_sample_sync_state():
    global tca_sync_epoch_published, tca_sync_go_released
    global tca_sync_pending_packet, tca_sync_pending_obssec
    tca_sync_epoch_published = False
    tca_sync_go_released = False
...
# clear live TCA values and plots for a new receiver run -----------------------
def reset_tca_live_display_state():
    global sol_log
    global tca_serial_rx_buf, tca_rx_line_count, tca_rx_parse_count
    global tca_rx_byte_count, tca_last_raw_rx_line, tca_last_raw_rx_bytes
    global tca_last_rx_parse_status
    global tca_last_tca_pos, tca_last_tca_vel, tca_last_tca_att
    global tca_last_tca_time, tca_last_tca_rx_wall
    global tca_last_pocket_pos, tca_last_pocket_vel
    global tca_realtime_h_error_m, tca_realtime_v_error_m, tca_realtime_3d_error_m
    global tca_realtime_error_text, tca_realtime_height_error_text
    global tca_realtime_3d_error_text
    ...
    tca_last_raw_rx_bytes = '-'
    tca_last_rx_parse_status = '-'
    tca_last_tca_pos = None
    tca_last_tca_vel = None
    tca_last_tca_att = None
    tca_last_tca_time = None
    tca_last_tca_rx_wall = 0.0
    tca_last_pocket_pos = None
    tca_last_pocket_vel = None
    tca_realtime_h_error_m = None
    tca_realtime_v_error_m = None
    tca_realtime_3d_error_m = None
    tca_realtime_error_text = '-'
    ...
    tca_status['tca_age'] = '-'
    if tca_page is not None:
        if hasattr(tca_page, 'error_samples'):
            tca_page.error_samples.clear()
        update_tca_page(tca_page)


def schedule_synced_receiver_autostart(bar):
    """Wait for prepared IMU arrays/serial, then start PocketSDR receiver."""
    if not POCKETSDR_AUTOSTART:
        return
    ...
    def poll_imu_ready():
        ...
        if POCKETSDR_SIM_READY_FILE:
            ready = os.path.abspath(os.path.expanduser(
                POCKETSDR_SIM_READY_FILE
            ))
            if not os.path.isfile(ready):
                status_bar_show('Waiting for IMU simulator READY...')
                root.after(POCKETSDR_SIM_SYNC_POLL_MS, poll_imu_ready)
                return

        status_bar_show(
            'IMU READY. Starting PocketSDR; waiting for first TCA epoch...'
        )
        print(
            'GNSS/IMU sync: IMU READY detected; starting PocketSDR receiver'
        )
        on_btn_start_push(bar)

    root.after(50, poll_imu_ready)

# Start button push callback ---------------------------------------------------
def on_btn_start_push(bar):
    global rcv_body
    if not rcv_body:
        reset_tca_live_display_state()
        reset_first_sample_sync_state()
        status_bar_show('')
        ...
        # Do not release the IMU here. rcv_open() is only receiver startup.
        # The first valid TCA obssec becomes the trajectory alignment reference.
...
# GNSS/TCA LLH CSV output options dialog ---------------------------------------
def tca_pvt_csv_opt_dlg(parent):
    global tca_pvt_csv_enabled, tca_pvt_csv_path

    if rcv_body:
        status_bar_show('Stop receiver before changing GNSS/TCA LLH CSV output.')
        return

    dlg = sdr_opt.modal_dlg_new(parent, 590, 190, 'GNSS/TCA LLH CSV Output')
    ena_var = BooleanVar(value=tca_pvt_csv_enabled)
    path_var = StringVar(value=tca_pvt_csv_path)

    chk = ttk.Checkbutton(dlg.panel, text='Store GNSS latitude/longitude/height and TCA latitude/longitude/height to CSV',
        variable=ena_var)
    chk.pack(anchor=W, padx=12, pady=(12, 8))

    row = Frame(dlg.panel)
    row.pack(fill=X, padx=12, pady=4)
    ttk.Label(row, text='CSV file', width=10, anchor=W).pack(side=LEFT)
    ent = ttk.Entry(row, textvariable=path_var, width=54, font=get_font())
    ent.pack(side=LEFT, expand=1, fill=X, padx=(2, 4))

    def browse_csv_file():
        path = filedialog.asksaveasfilename(parent=dlg.win,
            title='Save GNSS/TCA LLH CSV',
            initialfile=os.path.basename(path_var.get()) or 'gnss_tca_llh.csv',
            initialdir=os.path.dirname(os.path.abspath(path_var.get())) if path_var.get() else AP_DIR,
            defaultextension='.csv',
            filetypes=[('CSV files', '*.csv'), ('All files', '*.*')])
        if path:
            path_var.set(path)

    ttk.Button(row, text='Browse', width=8, command=browse_csv_file).pack(side=LEFT)

    ttk.Label(dlg.panel, text='One CSV row is written for each new PocketSDR GNSS FIX. Columns include GNSS lat/lon/height and latest TCA lat/lon/height.',
        anchor=W, foreground=P1_COLOR, wraplength=520).pack(fill=X, padx=12, pady=(8, 4))

    root.wait_window(dlg.win)
    if not dlg.ok:
        return

    new_enabled = bool(ena_var.get())
    new_path = path_var.get().strip()
    old_path = tca_pvt_csv_path

    if new_path:
        new_path = os.path.abspath(os.path.expanduser(new_path))

    tca_pvt_csv_enabled = new_enabled
    tca_pvt_csv_path = new_path
    tca_status['csv_status'] = 'ON' if tca_pvt_csv_enabled else 'OFF'
    tca_status['csv_file'] = tca_pvt_csv_path if tca_pvt_csv_path else '-'

    if old_path != tca_pvt_csv_path or not tca_pvt_csv_enabled:
        tca_pvt_csv_close()

    status_bar_show('GNSS/TCA LLH CSV output: %s' % ('ON' if tca_pvt_csv_enabled else 'OFF'))
...
def on_pages_timer(note, pages):
    ...
    update_rcv_log()
    update_sol_log()
    # Do not run TCA parsing/serial writing in the GUI page update cycle.
    # TCA transmission has its own 100 ms timer below.
...
# independent high-rate TCA return RX timer ------------------------------------
def on_tca_serial_rx_timer(root):
    # Teensy can return processed TCA/INS data at 200 Hz. Drain the serial RX
    # buffer frequently, but do not redraw the GUI here.
    if TCA_SERIAL_ENABLE:
        read_tca_serial_rx()
    root.after(max(1, TCA_SERIAL_RX_CYCLE), lambda: on_tca_serial_rx_timer(root))

# independent TCA serial/timer loop -------------------------------------------
def on_tca_serial_timer(root):
    # Timeout check stays here so a 20 s outage ends even if no new packet is
    # available. It is intentionally independent from the page redraw loop.
    if tca_outage_enabled():
        check_tca_outage_timeout()
    if rcv_body:
        update_tca_serial()
    root.after(max(5, TCA_SERIAL_CYCLE), lambda: on_tca_serial_timer(root))
...
def update_page(i, page):
    funcs = (update_rcv_page, update_rfch_page, update_bbch_page,
        ...
        update_log_page, update_tca_page)
    funcs[i](page)

# update pages -----------------------------------------------------------------
def pages_update(note, pages):
    global root_resize, rcv_body
    i = note.index('current')
    if not root_resize:
        update_page(i, pages[i])
    rcv_stat = get_rcv_stat(rcv_body)
    if rcv_body and rcv_stat.split()[1:2] == ['---']:
        rcv_close(rcv_body)
        rcv_body = None
        set_settings_btn_state(rcv_page, NORMAL)
        status_bar_show('IF file playback finished (end of file).')
    if rcv_body and not root_resize and i != 3:
        set_sel_ch(rcv_body, 0, 3e-6)
    text = 'Time: ' + rcv_stat.split()[0] + ' s'
...
def main():
    ...
    global root, stat_bar, opts_file

    if len(sys.argv) > 2:
        print('usage: pocket_sdr.py [settings_file]')
        return
    if len(sys.argv) == 2:
        opts_file = os.path.abspath(os.path.expanduser(sys.argv[1]))
    if not os.path.isfile(opts_file):
        print('settings file not found: %s' % opts_file)
        return
    ...
    # Synchronized mode replaces the manual Start click with an automatic start
    # after the IMU process has completed preprocessing and reported READY.
    schedule_synced_receiver_autostart(btn_bar)
...
#     
#     settings_file
#         Optional settings file to load instead of pocket_sdr.ini.
#
