...
# test INT16X2 format alias ----------------------------------------------------
def test_07():
    with tempfile.NamedTemporaryFile(delete=False) as fp:
        np.array([1, -2, 3, 4], dtype='int16').tofile(fp)
        path = fp.name

    data = sdr_func.read_data(path, 1.0, 2, 2.0, 0.0, fmt='INT16X2')
    ...
    if np.allclose(data, expected):
        ...
        print('test_07: NG')
        raise AssertionError('INT16X2 alias did not decode as int16 complex data')
