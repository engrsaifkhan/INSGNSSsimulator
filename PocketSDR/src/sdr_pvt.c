...
/* TCA feature export bridge implemented in pntpos.c.
 *
 * pntpos_export_tca_features() must compute TCA rows directly from obs+nav+
 * reference receiver ECEF position and write the same buffer returned by
 * pntpos_get_tca_csv(). It is called from update_sol() after the normal PVT
 * solution attempt so PVT generation remains unchanged and cannot be disturbed
 * by the TCA export path.
 */
extern const char *pntpos_get_tca_csv(void);
extern int pntpos_export_tca_features(const obsd_t *obs, int n,
    const nav_t *nav, const prcopt_t *opt, const double *rr_ref);
...
/* PVT stability gate. This does not smooth a valid solution. It only rejects
 * impossible/marginal jumps and lets the existing last-valid-position hold path
 * keep the display/reference steady until a clean fix returns.
 */
#ifndef SDR_PVT_MIN_NS
#define SDR_PVT_MIN_NS          5      /* minimum accepted satellites */
#endif
#ifndef SDR_PVT_ALT_ABS_MAX_M
#define SDR_PVT_ALT_ABS_MAX_M   20000.0
#endif
#ifndef SDR_PVT_HSTEP_MIN_M
#define SDR_PVT_HSTEP_MIN_M     12.0   /* static/slow lower horizontal gate */
#endif
#ifndef SDR_PVT_VSTEP_MIN_M
#define SDR_PVT_VSTEP_MIN_M     25.0   /* static/slow lower vertical gate */
#endif
#ifndef SDR_PVT_MAX_SPEED_MPS
#define SDR_PVT_MAX_SPEED_MPS   80.0   /* dynamic horizontal gate = speed*dt */
#endif
#ifndef SDR_PVT_MAX_VSPEED_MPS
#define SDR_PVT_MAX_VSPEED_MPS  30.0   /* dynamic vertical gate = speed*dt */
#endif
...
// remap observation data to frequency index 0 --------------------------------
//
// The RTKLIB/PocketSDR pntpos() and the TCA feature exporter read P[0], D[0],
// code[0] and SNR[0]. Some signals can appear in index 1..NFREQ+NEXOBS-1.
// This function promotes the first valid available measurement to index 0 when
// index 0 is empty. It does not modify an observation that already has P[0].
static void remap_obs_freq0(obs_t *obs, const nav_t *nav)
{
    (void)nav;
    ...
    for (int i = 0; i < obs->n; i++) {
        ...
        if (data->P[0] > 0.0 && data->code[0]) continue;

        int best = -1;
        for (int j = 1; j < NFREQ + NEXOBS; j++) {
            if (data->P[j] > 0.0 && data->code[j]) {
                best = j;
                break;
            }
        }
        ...
        data->P[0]    = data->P[best];
        data->L[0]    = data->L[best];
        data->D[0]    = data->D[best];
        data->SNR[0]  = data->SNR[best];
        data->LLI[0]  = data->LLI[best];
        data->code[0] = data->code[best];
        ...
        sdr_log(4, "$LOG,OBS REMAP sat=%d freq_idx=%d -> 0 code=%d",
            data->sat, best, data->code[0]);
    }
}

// reject impossible/marginal PVT fixes without smoothing ----------------------
static int reject_unstable_pvt(sdr_pvt_t *pvt, const sol_t *sol, double time,
    char *msg, int msg_size)
{
    double pos[3],pos0[3],dr[3],enu[3],dt,hstep,vstep,hgate,vgate;

    if (!sol || !sol->stat) return 1;
    ...
    if (!pvt->last_valid || norm(pvt->last_rr,3) <= 0.0) return 0;

    dt=fabs(timediff(sol->time,pvt->last_time));
    if (dt < SDR_CYC) dt = sdr_epoch > SDR_CYC ? sdr_epoch : SDR_CYC;

    for (int i=0;i<3;i++) dr[i]=sol->rr[i]-pvt->last_rr[i];
    ecef2pos(pvt->last_rr,pos0);
    ecef2enu(pos0,dr,enu);

    hstep=sqrt(enu[0]*enu[0]+enu[1]*enu[1]);
    vstep=fabs(enu[2]);
    hgate=fmax(SDR_PVT_HSTEP_MIN_M, SDR_PVT_MAX_SPEED_MPS*dt);
    vgate=fmax(SDR_PVT_VSTEP_MIN_M, SDR_PVT_MAX_VSPEED_MPS*dt);

    if (hstep > hgate || vstep > vgate) {
...
static void update_sol(sdr_pvt_t *pvt)
{
    ...
    /*
     * Remap observations before both paths:
     *   - normal pntpos() PVT path
     *   - independent TCA feature exporter path
     *
     * This is required because both paths use P[0], D[0], code[0], SNR[0].
     */
    remap_obs_freq0(pvt->obs, pvt->nav);

    /*
     * Keep the normal PVT input behavior close to the original file:
     * delete duplicated observations by satellite and skip invalid P[0].
     * This preserves the PocketSDR PVT mechanism.
     */
    ...
    /*
     * Normal PocketSDR PVT generation.
     * Do not use TCA logic to decide PVT status.
     */
    if (nobs > 0) {
        stat = pntpos(obs, nobs, pvt->nav, &opt, pvt->sol, NULL,
            pvt->ssat, msg);
    }
    else {
        pvt->sol->stat = SOLQ_NONE;
        pvt->sol->ns = 0;
        strcpy(msg, "no valid pseudorange observation");
    }
    ...
    if (stat) {
        ...
        /* Reject impossible/marginal PVT jumps before logging or updating
         * last_rr. This is a hold/reject gate, not a smoother. It prevents a
         * single poor epoch from becoming the new PocketSDR PVT reference.
         */
        if (reject_unstable_pvt(pvt, pvt->sol, time, msg, sizeof(msg))) {
            sdr_log(3, "$LOG,%.3f,PNTPOS HOLD,%s", time, msg);
            pvt->sol->stat = SOLQ_NONE;
            pvt->sol->ns = 0;
            stat = 0;
        }
    ...
    if (stat) {
        // save last fix for fast acquisition and TCA reference position
        ...
        /*
         * PVT failed. Keep the PVT failure behavior, but do not clear or block
         * the independent TCA stream. If we have a previous valid position,
         * keep it in sol->rr so az/el display and TCA reference remain usable.
         */
        pvt->sol->ns = 0;

        if (pvt->last_valid && norm(pvt->last_rr, 3) > 0.0) {
            pvt->sol->rr[0] = pvt->last_rr[0];
            pvt->sol->rr[1] = pvt->last_rr[1];
            pvt->sol->rr[2] = pvt->last_rr[2];
    ...
    /*
     * Independent TCA feature generation from obs + nav.
     *
     * This call is intentionally AFTER pntpos(): if old pntpos() logic clears
     * the TCA buffer on PVT failure, this exporter overwrites the buffer again
     * with any valid compensated TCA rows.
     *
     * It must not modify pvt->sol, pvt->ssat or PVT status. It should only
     * write the TCA CSV buffer returned by pntpos_get_tca_csv().
     */
    if (pvt->obs->n > 0 && pvt->last_valid && norm(pvt->last_rr, 3) > 0.0) {
        int n_tca;
        const double *rr_ref = pvt->last_rr;
        const double *rr_vel = NULL;

        /* If the current epoch solved, use the full 6-state sol->rr.
         * In this no-export-correction version the velocity argument is kept
         * only for API compatibility with pntpos_export_tca_features6().
         * If not solved, fall back to last_rr position only.
         */
        if (stat && norm(pvt->sol->rr, 3) > 0.0) {
            rr_ref = pvt->sol->rr;
            rr_vel = pvt->sol->rr + 3;
        }

        n_tca = pntpos_export_tca_features6(pvt->obs->data, pvt->obs->n,
            pvt->nav, &opt, rr_ref, rr_vel);

        sdr_log(4, "$LOG,%.3f,TCA_EXPORT OBS_N=%d ROWS=%d PVT_STAT=%d", time,
            pvt->obs->n, n_tca, stat);
    }
    else {
        sdr_log(4, "$LOG,%.3f,TCA_EXPORT SKIP OBS_N=%d LAST_VALID=%d", time,
            pvt->obs->n, pvt->last_valid);
    }
...
//------------------------------------------------------------------------------
//  Get TCA feature CSV string.
//
//  The returned string is generated by pntpos.c / pntpos_export_tca_features()
//  and transmitted by python/pocket_sdr.py through POCKETSDR_TCA_PORT.
//
